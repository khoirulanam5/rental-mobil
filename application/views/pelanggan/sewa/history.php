    ...
    <div class="modal-dialog ">
      ...
      <form id="FRM_UPLOAD" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-body">
            ...
            <div class="d-md-flex mt-2">
                <div class="form-group col-6 col-md-6">
                    <label for="" class="label">Masukan Pembayaran</label>
  ...
  function REFRESH_DATA() {
    ...
    var tb_data = $("#tb_history").DataTable({
        ...
        "columns": [
            ...
            { "data": null, "render": function(data) { return data.nm_jenis_mobil }, className: "text-center" },
            { 
            ...
            { 
                ...
                "render": function(data) {
                    ...
                    // Prioritaskan nilai DP dari database jika ada
                    if (data.dp_total_pembayaran && data.dp_total_pembayaran > 0) {
                        nominalDP = data.dp_total_pembayaran;
                    } 
                    // Jika status_validasi adalah DP_UPLOAD dan DP dari database kosong, hitung 50% dari total_pembayaran
                    else if (data.status_validasi === 'DP_UPLOAD' && data.total_pembayaran) {
                        nominalDP = (data.total_pembayaran * 0.5).toFixed();
                    }

                    return `Rp ${parseFloat(nominalDP).toLocaleString("id-ID")}`;
                }
            },
...
function editData(data, index) {
    ...
    let fullPayment = parseFloat(data.total_pembayaran) || 0; // Ambil total pembayaran penuh
    ...
    $("[name='total_pembayaran']").val(fullPayment.toFixed(0));
    ...
    $("input[name='jenis_pembayaran']").on("change", function () {
        ...
        if (jenisPembayaran === "dp") {
            $("[name='total_pembayaran']").val((fullPayment * 0.5).toFixed());
        } else {
            $("[name='total_pembayaran']").val(fullPayment.toFixed());
        }
    });
}
