...
class Pembayaran extends CI_Controller {
  ...
  public function sendNotif(){
    $data = $this->db->query("
      SELECT A.id_penyewaan, B.no_pelanggan, B.nm_pelanggan, B.tgl_penyewaan FROM tb_pembayaran A
      INNER JOIN tb_penyewaan B ON A.id_penyewaan = B.id_penyewaan
      WHERE A.id_pembayaran='".$this->input->post('id_pembayaran')."'
    ")->result_array();

    $no_pelanggan = $data[0]['no_pelanggan'];
    $nm_pelanggan = $data[0]['nm_pelanggan'];
    $tgl_penyewaan = $data[0]['tgl_penyewaan'];
    $id_penyewaan = $data[0]['id_penyewaan'];

    $this->db->query("
      UPDATE tb_penyewaan SET notif_wa='TERKIRIM' WHERE id_penyewaan = '".$id_penyewaan."'
    ");
    ...
    curl_setopt($curlHandle, CURLOPT_POSTFIELDS, array(
        ...
        'passkey' => $passkey,
        'to' => $no_pelanggan,
        'message' => "Kepada saudara ".$nm_pelanggan." yang terhormat. Kami dari Penyu Mobil, mengingatkan untuk penyewaan mobil anda pada ".$tgl_penyewaan.". Mohon untuk dapat dapat datang ke tempat tepat waktu. Terimakasih atas perhatian dan kerjasamanya, Salam"
    ));
